...
```{r Exercise 1.9}
# write code here
ifelse(mtcars$am == 1, mtcars$transmission <- "manual", mtcars$transmission <- "automatic")  
str(mtcars$transmission)
str(mtcars)
...
```{r}
write.csv(clinic, "~/490_cluster/analysis_databrca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("~/490_cluster/analysis_databrca_clinical_data.csv")
```
...
```{r Exercise 4.1}
# create plot here
plot(age_at_diagnosis ~ death_days_to,
     data = clinic,
     xlab = "Days to Death",
...
```{r Exercise 4.2}
boxplot(days_to_initial_pathologic_diagnosis ~ last_contact_days_to,
     data = clinic,
     xlab = "Days to Initial Diagnosis to Last contacted date",
     ylab = "days_to_initial_pathologic_diagnosis",
     main = "days_to_initial_pathologic_diagnosis vs. last_contact_days_t")
...
```{r}
under30_over60_mask <- ifelse(clinic$age_at_diagnosis < 35,
                              TRUE, 
                              ifelse(clinic$age_at_diagnosis > 60, TRUE, FALSE))
```
...
cleaned_clinic$age_category <- ifelse(
  is.na(cleaned_clinic$age_at_diagnosis), NA_character_,
  ifelse( cleaned_clinic$age_at_diagnosis <= 35, "Young", 
...
```{r}
cleaned_clinic$survival_time <- !is.na(clinic$last_contact_days_to)
```
...
KM_plot <- survplot$plot + theme_bw() + theme(axis.title = element_text(size=24), 
...
Middle appears to have the highest rate of survival
# 4. What age group appears to have the worst rate of survival? 
Young Appears to have the worst rate of survival
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
I am unable to discern this answer through what is given
